...
def test_get_data_from_weather_api(mock_get):
    ...
    mock_get.return_value.json.return_value = {
        "properties": {
            "forecastHourly": "https://api.weather.gov/gridpoints/LSX/27,123/forecast/hourly"
        }
    }
    ...
    assert (
        actual_output == excepted_output
    ), f"Expected:{excepted_output} differs from actual value {actual_output}"
